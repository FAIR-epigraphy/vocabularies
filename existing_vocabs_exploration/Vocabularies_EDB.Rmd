---
title: "Use of the existing Eagle project vocabularies in the EDB dataset"
author:
...
```{r}
dir.create("./data/EDB")
zen4R::download_zenodo("10.5281/zenodo.3575485", path = "./data/EDB")
```

## Extract the Zenodo repository
```{r}

zip_files <- list.files(path = "./data/EDB/", pattern = "*.zip", full.names = TRUE)

lapply(zip_files, unzip, exdir = "./data/EDB/")

unzip("./data/EDB/*.zip", exdir = "./data/EDB/")
...
EDR_csv <- read.csv("./xml_to_csv/EDB_type_of_inscriptions.csv")
...
# Explore the `EDB` vocabularies and its use
...
```{r}
write_csv(EDR_simple_overview, "./output/EDB_inscription_type_from_XML.csv", col_names = TRUE)
```
